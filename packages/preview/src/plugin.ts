...
import type { MarkdownItPreviewOptions } from "./options.js";

const TAB_MARKER = `@tab`;

const getPreviewRule =
...
export const tab: PluginWithOptions<MarkdownItPreviewOptions> = (
  md,
  options,
) => {
  const {
    previewOpenRenderer = (
      ...
      token.attrJoin("class", `preview-wrapper`);
...
`;
    ...
    previewCloseRenderer = (): string => `</div>`,
  } = options ?? {};
