 ...
 * 非嵌套内联规则选项（线性扫描）
 *
 * Uses a high-performance linear scanner. No inline tags (bold, italic, etc.) are parsed inside the
 * markers. Best for simple tags like sub/sup. 使用高性能线性扫描器。 标记内部不会解析内联标签（粗体、斜体等）。 最适合 sub/sup 等简单标签。
 */
