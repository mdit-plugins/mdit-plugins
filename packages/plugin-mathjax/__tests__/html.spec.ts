...
import { createMathjaxInstance, getDocumentOptions, mathjax } from "../src/index.js";
...
`),
  ...
  it("should log debug message in asyncLoad", async () => {
    const spy = vi.spyOn(console, "debug").mockImplementation(() => {});

    await getDocumentOptions({ debug: true, output: "chtml" });

    try {
      await mathjaxLib.asyncLoad("test-module.js");
    } catch {
      /* import will fail for non-existent module */
    }

    expect(spy).toHaveBeenCalledWith("[MathJax]: loading test-module.js...");
